...
/// startHour/startMinute/endHour/endMinute. Overnight windows are valid.
class LoadSchedule {
  const LoadSchedule({
    this.enabled = false,
    this.startHour,
  ...
  Map<String, dynamic> toWireJson() {
    ...
    // Firmware rejects a zero-length enabled window.
...
class LoadModel {
  ...
  /// Current requested/planned state encoded by the firmware mode.
  ...
  bool get available => owningNodeMac.isNotEmpty && relayPin >= 0;
  bool? get displayState => requestedState;
  ...
  factory LoadModel.fromJson(Map<String, dynamic> json) {
    ...
    final scheduleJson =
        json.mapOrNull('schedule') ?? const <String, dynamic>{};
    ...
    return LoadModel(
      ...
      requestedState: _stateFromMode(wireMode),
      schedule: LoadSchedule(
        enabled: scheduleJson.boolOrNull('enabled') ?? false,
        startHour: scheduleJson.intOrNull('startHour'),
        startMinute: scheduleJson.intOrNull('startMinute'),
      ...
      ratedPowerW: json.doubleOrNull('powerRatingWatts'),
